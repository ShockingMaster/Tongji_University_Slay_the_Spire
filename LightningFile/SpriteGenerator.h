...
class RelicSpriteGenerator {
public:
    static Sprite* createCardSprite(const std::shared_ptr<Relic>& Relic) {
        // 创建药水的卡片精灵
        std::string Name = Relic->name_;
        auto Painting = Sprite::create(Name + ".png");
        ...
        // 创建描述文本的Label，默认隐藏
        auto descriptionLabel = Label::createWithSystemFont(Relic->description_, "Arial", 18);
        descriptionLabel->setPosition(Vec2(Painting->getContentSize().width / 2, -20)); // 放在Painting下方
        descriptionLabel->setVisible(false);
        descriptionLabel->setName("descriptionLabel");
